...
export function stripMarkdownForPreview(line: string): string {
  return line
    .replace(/^#{1,6}\s+/, "")
    .replace(/^\s*[-*+]\s+/, "")
    .replace(/^\s*\d+\.\s+/, "")
...
export function groupRunsByDay(runs: AutomationRunRecord[]): Array<{ label: string; runs: AutomationRunRecord[] }> {
